...
ways of accessing `arcpy` in Python:
...
```{r}
attach(arcpy_env("Delete_management", NULL))
...
The `arcpyr` package is great for accessing ArcGIS toolboxes, but it 
can't handle the object-oriented aspects of `arcpy` and Python. If you
need things like raster calculations and attribute table reading and 
modification, check out the `arcpyrextra` package. 